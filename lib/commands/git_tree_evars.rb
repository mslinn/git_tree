...
module GitTree
  ...
  class EvarsCommand < AbstractCommand
    ...
    def run
      result = []
      if @args.empty?
        # No args provided, use default roots from GitTreeWalker
        walker = GitTreeWalker.new([], verbosity: @options[:verbosity])
        walker.find_and_process_repos do |dir|
          # We can't easily construct the env var reference, so we just use the full path.
          # This is a reasonable compromise for the convenience of no-arg usage.
